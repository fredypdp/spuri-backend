...
func buildCredential(in CredencialInput) (CredencialAppyPay, error) {
	if in.ContextoTipo == ContextoSpuri {
		if err := applySpuriEnvDefaults(&in); err != nil {
...
func applySpuriEnvDefaults(in *CredencialInput) error {
	setDefault(&in.AuthBaseURL, "APPYPAY_AUTH_BASE_URL")
	setDefault(&in.APIBaseURL, "APPYPAY_API_BASE_URL")
	setDefault(&in.WebAPIBaseURL, "APPYPAY_WEBAPI_BASE_URL")
	setDefault(&in.ClientID, "APPYPAY_SPURI_CLIENT_ID")
	setDefault(&in.ClientSecret, "APPYPAY_SPURI_CLIENT_SECRET")
	setDefault(&in.Resource, "APPYPAY_SPURI_RESOURCE")
	setDefault(&in.WebhookSecret, "APPYPAY_SPURI_WEBHOOK_SECRET")
	if len(in.Applications) == 0 {
		appsJSON := os.Getenv("APPYPAY_SPURI_APPLICATIONS_JSON")
		if appsJSON != "" {
			if err := json.Unmarshal([]byte(appsJSON), &in.Applications); err != nil {
				return fmt.Errorf("APPYPAY_SPURI_APPLICATIONS_JSON inválido: %w", err)
			}
		}
	}
	return nil
...
func setDefault(dst *string, envName string) {
	if *dst == "" {
