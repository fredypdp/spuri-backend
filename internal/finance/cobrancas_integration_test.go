...
func TestIntegrationListCobrancasFiltraOrigemEstadoEIsolaPorAcademia(t *testing.T) {
	...
	todas, err := service.ListCobrancas(ctx, ContextoAcademia, academiaA, nil, nil, nil, nil, "", "", nil, 50, 0)
	if err != nil {
		t.Fatal(err)
	}
	if todas.Total != 3 {
	...
	pagas, err := service.ListCobrancas(ctx, ContextoAcademia, academiaA, []string{"Success"}, nil, nil, nil, "", "", nil, 50, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	matriculas, err := service.ListCobrancas(ctx, ContextoAcademia, academiaA, nil, []string{"matricula"}, nil, nil, "", "", nil, 50, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	pagina, err := service.ListCobrancas(ctx, ContextoAcademia, academiaA, nil, nil, nil, nil, "", "", nil, 1, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	outraAcademia, err := service.ListCobrancas(ctx, ContextoAcademia, academiaB, nil, nil, nil, nil, "", "", nil, 50, 0)
	if err != nil {
		t.Fatal(err)
	}
