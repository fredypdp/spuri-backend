...
func (p *AcademiaProjection) handleAcademiaCriada(event db.Event) error {
	...
	query := fmt.Sprintf(`
		INSERT INTO projection_academias (
			...
			email_verificado, version, created_at, updated_at, last_event_id
		) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', %s, %s, %s, %s, 
			'inativo', '%s', FALSE, %d, '%s', CURRENT_TIMESTAMP, '%s')
		ON CONFLICT (id) DO UPDATE SET
			type = EXCLUDED.type, nome = EXCLUDED.nome, codigo_academia = EXCLUDED.codigo_academia,
			senha_hash = EXCLUDED.senha_hash, provincia = EXCLUDED.provincia,
			endereco = EXCLUDED.endereco, numero_telefone = EXCLUDED.numero_telefone,
			email = EXCLUDED.email, website = EXCLUDED.website, nivel_escolar = EXCLUDED.nivel_escolar,
			cursos = EXCLUDED.cursos, version = EXCLUDED.version,
			updated_at = EXCLUDED.updated_at, last_event_id = EXCLUDED.last_event_id
	`, event.AggregateID, db.SafeString(payload.Type), db.SafeString(payload.Nome),
		...
		db.SafeString(string(cursosJSON)), event.EventVersion,
		payload.CreatedAt.Format(time.RFC3339), event.EventID)
...
func (p *AcademiaProjection) handleAcademiaDadosAtualizados(event db.Event) error {
	var payload struct {
		Nome, Provincia, Endereco, NumeroTelefone, Email, Website, NivelEscolar *string
		Cursos                                                                   []string
		EmailAlterado                                                            bool
	}
	...
	for field, value := range updates {
		if value != nil {
			query := fmt.Sprintf(`UPDATE projection_academias SET %s = '%s' WHERE id = '%s'`,
	...
	if payload.Email != nil {
		...
		query := fmt.Sprintf(`UPDATE projection_academias SET email = '%s', email_verificado = %s WHERE id = '%s'`,
	...
	if payload.Cursos != nil {
		cursosJSON, _ := json.Marshal(payload.Cursos)
		query := fmt.Sprintf(`UPDATE projection_academias SET cursos = '%s' WHERE id = '%s'`,
			db.SafeString(string(cursosJSON)), event.AggregateID)
		p.client.DB().Exec(query)
	}
...
func (p *AcademiaProjection) queryAcademia(whereClause string) (*AcademiaDTO, error) {
	...
	err := p.client.DB().QueryRow(query).Scan(
		...
		&dto.Website, &dto.NivelEscolar, &dto.Status, &cursosJSON,
