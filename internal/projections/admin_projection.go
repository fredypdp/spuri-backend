...
func (p *AdminProjection) Rebuild() error {
	log.Printf("[ADMIN_PROJECTION] Rebuild iniciado")
	...
	log.Printf("[ADMIN_PROJECTION] Rebuild concluído: %d eventos", eventCount)
	return rows.Err()
}
...
func (p *AdminProjection) handleAdminDadosAtualizados(event db.Event) error {
	...
	if payload.Email != nil {
		...
		if payload.EmailAlterado {
			query := fmt.Sprintf(`UPDATE projection_admins SET email = '%s', email_verificado = FALSE WHERE id = '%s'`, safe, aggID)
			p.client.DB().Exec(query)
		} else {
...
func (p *AdminProjection) GetByID(id uuid.UUID) (*AdminDTO, error) {
	...
	err := p.client.DB().QueryRow(query).Scan(
		&dto.ID, &dto.Nome, &dto.Email, &dto.SenhaHash, &dto.Role, &dto.Status, &dto.EmailVerificado,
