...
func (p *AvaliacaoFinalProjection) Handle(event db.Event) error {
	...
	if event.EventType == "AvaliacaoFinalAnoAcademico" || event.EventType == "AvaliacaoFinalEscolar" || event.EventType == "AvaliacaoFinalSuperior" {
...
func (p *AvaliacaoFinalProjection) Rebuild() error {
	...
	rows, err := p.client.DB().Query(`
		...
		FROM spuri_ledger
		WHERE aggregate_type = 'Estudante' AND event_type IN ('AvaliacaoFinalAnoAcademico','AvaliacaoFinalEscolar','AvaliacaoFinalSuperior')
		ORDER BY id ASC
	`)
	if err != nil {
		return err
