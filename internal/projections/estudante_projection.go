...
func (p *EstudanteProjection) Rebuild() error {
	log.Printf("[ESTUDANTE_PROJECTION] Rebuild iniciado")
	...
	log.Printf("[ESTUDANTE_PROJECTION] Rebuild concluído: %d eventos", eventCount)
	return rows.Err()
}
...
func (p *EstudanteProjection) handleEstudanteCriado(event db.Event) error {
	...
	var emailStr, telefoneStr, bilheteStr, bilheteRespStr string
