...
func TestCorrigirFaltaExigeMotivo(t *testing.T) {
	...
	if err := e.RegistrarFalta(academia, "2026", "7_ano_fundamental", data, materia, 1, nil, uuid.New(), MaxQuantidadeFaltasPadrao); err != nil {
		t.Fatal(err)
	}
	if err := e.CorrigirFalta(uuid.New(), academia, "2026", data, materia, 2, nil, "", uuid.New(), MaxQuantidadeFaltasPadrao); err == nil {
...
func TestRegistrarECorrigirFaltaRespeitamTetoDoAggregate(t *testing.T) {
	...
	materia := uuid.New()
	data := time.Date(2026, 2, 1, 0, 0, 0, 0, time.UTC)
	if err := e.RegistrarFalta(academia, "2026", "7_ano_fundamental", data, materia, MaxQuantidadeFaltasPadrao+1, nil, uuid.New(), MaxQuantidadeFaltasPadrao); err == nil {
		t.Fatal("falta acima do teto deveria ser rejeitada pelo aggregate")
	}
	if err := e.RegistrarFalta(academia, "2026", "7_ano_fundamental", data, materia, 1, nil, uuid.New(), MaxQuantidadeFaltasPadrao); err != nil {
		t.Fatal(err)
	}
	if err := e.CorrigirFalta(uuid.New(), academia, "2026", data, materia, MaxQuantidadeFaltasPadrao+1, nil, "ajuste", uuid.New(), MaxQuantidadeFaltasPadrao); err == nil {
