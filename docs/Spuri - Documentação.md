...
- **Admin FPP** define o **ano letivo oficial global do sistema** via `POST /admin/definir-ano-letivo-geral`; quando já houver academias com ano letivo, o global deve ser maior que o maior ano letivo ativo dessas academias
- **Academia** define diretamente o seu ano letivo ativo via `POST /academia/definir-ano-letivo` apenas se ainda não tiver ano letivo, sempre alinhado ao global; depois avança exclusivamente pela finalização em `POST /academia/anos-letivos/finalizar`
...
O Admin FPP define o global diretamente e não há mais uma rota de “ano letivo seguinte”. A evolução operacional é feita pelas academias: ao finalizar o ano letivo ativo, o backend calcula o próximo período a partir do ano final do anterior e atualiza o ano letivo da academia. Quando todas as academias ativas ficarem no mesmo ano letivo, o sistema sincroniza automaticamente esse ano como global. O ano letivo ativo é resolvido automaticamente em todos os novos registos de nota, falta e avaliação.
...
| Admin FPP define o ano letivo oficial global via `POST /admin/definir-ano-letivo-geral`; se existir ano letivo em academias, o novo global deve ser maior que o maior deles | É a referência obrigatória para academias sem ano letivo |
| Academia só pode definir diretamente quando ainda não possui ano letivo e sempre igual ao oficial global; avanços usam `POST /academia/anos-letivos/finalizar` | Divergência é bloqueada com erro de negócio             |
...
As academias podem declarar a finalização de um ano letivo por tipo. Essa ação é registrada no ledger por meio do evento `AnoLetivoAcademiaFinalizado` e projetada em `projection_anos_letivos_academia_finalizacoes`, mantendo a informação auditável por academia, tipo, ano, usuário, data e observação. A finalização com avanço automático para o ano letivo seguinte só é aceita na janela operacional entre o mês de fim do período letivo configurado para o tipo e o mês imediatamente anterior ao mês de início do próximo período: em termos de validação, o mês atual precisa ser `>=` ao mês final de `periodo` e `<` ao mês inicial de `periodo`. Exemplo: com `periodo=10_07`, a academia pode finalizar em julho, agosto ou setembro (meses 07, 08 e 09); de outubro a junho a operação é bloqueada, porque o ano letivo ainda está em curso ou o próximo período já começou. Quando todas as academias ativas passam a estar no mesmo ano letivo, a plataforma define automaticamente esse ano como global; definições administrativas diretas continuam bloqueadas se forem menores ou iguais ao maior ano letivo já ativo em alguma academia.
