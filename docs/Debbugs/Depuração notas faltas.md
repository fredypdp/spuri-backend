...
2. Novo método de comando no aggregate, ex. `(e *Estudante) CorrigirNota(chaveOriginal, novaNota, motivo, corrigidoPor, ...)`, que **não apaga** o evento original — apenas emite um evento de correção referenciando-o. Regras de negócio a impor no aggregate:
   ...
   - (Opcional, decisão de produto) limite de janela de tempo para autocorreção pela própria academia (ex. até N dias após o lançamento original) — depois disso, exigir aprovação de admin.
3. Rotas novas: `PATCH /academia/notas-aluno/:id` e `PATCH /academia/faltas-aluno/:id` (ou `POST .../corrigir`), restritas a `academia` (dona do registro) e `admin`, com `motivo` obrigatório no corpo.
