...
CREATE TABLE IF NOT EXISTS projection_estudantes (
    ...
    status VARCHAR(20) DEFAULT 'inativo' CHECK (status IN ('inativo', 'ativo', 'pendente_documentos')),
...
CREATE TABLE IF NOT EXISTS projection_academias (
    ...
    status VARCHAR(20) DEFAULT 'inativo' CHECK (status IN ('ativo', 'inativo', 'deletado')),
...
CREATE TABLE IF NOT EXISTS projection_materias (
    ...
    pendencia_nivel_conclusao VARCHAR(50) DEFAULT NULL,
    status VARCHAR(20) DEFAULT 'ativo' CHECK (status IN ('ativo', 'inativo')),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    version INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_materias_pendencia_nivel_conclusao ON projection_materias(pendencia_nivel_conclusao) WHERE pendencia_nivel_conclusao IS NOT NULL;
...
COMMENT ON COLUMN projection_estudantes.status IS 'Status geral: inativo, ativo, pendente_documentos';
